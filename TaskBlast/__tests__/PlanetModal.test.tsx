...
describe("PlanetModal", () => {
  ...
  it("renders planet data when visible", async () => {
    ...
    (getDoc as jest.Mock).mockResolvedValueOnce({
      exists: () => true,
      data: () => ({ name: "Glowy", description: "A glowing world", cost: 5 }),
    });
  ...
  it("allows unlocking when user has enough crystals", async () => {
    ...
    (getDoc as jest.Mock).mockResolvedValueOnce({
      exists: () => true,
      data: () => ({ name: "UnlockMe", description: "Buy me", cost: 10 }),
    });
    ...
    // Press Confirm Unlock -> should call unlockPlanet service
    fireEvent.press(screen.getByTestId("confirm-unlock-button"));

    await waitFor(() => {
      expect(unlockPlanet).toHaveBeenCalled();
    });
  });
  ...
  it("handles planet load errors", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("Failed to load planet")).toBeTruthy(),
    );
  });
  ...
  it("close button invokes onClose", async () => {
    (getDoc as jest.Mock).mockResolvedValueOnce({
      exists: () => true,
      data: () => ({ name: "CloseMe", description: "desc" }),
    });
  ...
  it("shows 'No planet data found' when doc exists=false", async () => {
    ...
    await waitFor(() =>
      expect(screen.getByText("No planet data found")).toBeTruthy(),
    );
  });
  ...
  it("alerts on unlock failure when setDoc rejects", async () => {
    ...
    (getDoc as jest.Mock).mockResolvedValueOnce({
      exists: () => true,
      data: () => ({ name: "FailUnlock", description: "desc", cost: 5 }),
    });
    (getDoc as jest.Mock).mockResolvedValueOnce({
      exists: () => true,
      data: () => ({ rocks: 100 }),
    ...
    (unlockPlanet as jest.Mock).mockRejectedValueOnce(new Error("write fail"));
